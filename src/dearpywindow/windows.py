...
def main_window(
    ...
    with dpg.window(label="Epicycles", tag="main_window"):
        ...
        dpg.add_button(
            label="Load json config", callback=lambda: dpg.show_item("load_file")
        )
        ...
        dpg.add_button(
            label="Dump config to json", callback=lambda: dpg.show_item("dump_file")
        )
...
def create_modal_list_points(width: int, height: int):
    ...
    with dpg.window(tag="list_points_modal", label="List Points", autosize=True):
        ...
        dpg.add_button(
            ...
            callback=lambda: object_actions.delete_point_callback(
        ...
        dpg.add_slider_int(
            label="x", min_value=0, max_value=width, tag="x", enabled=False
        )
        dpg.add_slider_int(
            label="y", min_value=0, max_value=height, tag="y", enabled=False
        )
        dpg.add_slider_int(
            label="speed", min_value=-1000, max_value=1000, tag="speed", enabled=False
        )
        dpg.add_slider_int(
            label="radius", min_value=1, max_value=1000, tag="radius", enabled=False
        )
        dpg.add_slider_int(
            label="size", min_value=2, max_value=300, tag="size", enabled=False
        )
...
def create_modal_create_point(width: int, height: int):
    ...
    with dpg.window(
        ...
        dpg.add_slider_int(
            label="speed", min_value=-1000, max_value=1000, tag="create_speed"
        )
        dpg.add_slider_int(
            label="radius", min_value=1, max_value=1000, tag="create_radius"
        )
        dpg.add_slider_int(
            label="size", min_value=1, max_value=300, tag="create_size", default_value=1
        )
...
def create_modal_update_point(width: int, height: int):
    ...
    with dpg.window(
        ...
        dpg.add_slider_int(
            label="speed", min_value=-1000, max_value=1000, tag="update_speed"
        )
        dpg.add_slider_int(
            label="radius", min_value=1, max_value=1000, tag="update_radius"
        )
        dpg.add_slider_int(
            label="size", min_value=1, max_value=300, tag="update_size", default_value=1
        )

        dpg.add_button(
            label="Update",
            callback=lambda: object_actions.update_point(object_actions.get_point()),
        )
...
def create_modal_create_child(width: int, height: int):
    ...
    with dpg.window(
        ...
        dpg.add_combo(
            config.Colors.get_colors(),
            label="Choose color",
            tag="create_child_color",
            default_value="WHITE",
        )

        dpg.add_slider_int(
            label="speed", min_value=-1000, max_value=1000, tag="create_child_speed"
        )
        dpg.add_slider_int(
            label="radius", min_value=1, max_value=1000, tag="create_child_radius"
        )
        dpg.add_slider_int(
            label="size",
            min_value=1,
            max_value=300,
            tag="create_child_size",
            default_value=1,
        )

        dpg.add_button(
            label="Create child",
            callback=lambda: object_actions.create_child(object_actions.get_point()),
        )
...
def create_modal_update_child(width: int, height: int):
    ...
    with dpg.window(
        ...
        dpg.add_combo(
            config.Colors.get_colors(),
            label="Choose color",
            tag="update_child_color",
            default_value="WHITE",
        )

        dpg.add_slider_int(
            label="speed", min_value=-1000, max_value=1000, tag="update_child_speed"
        )
        dpg.add_slider_int(
            label="radius", min_value=1, max_value=1000, tag="update_child_radius"
        )
        dpg.add_slider_int(
            label="size",
            min_value=1,
            max_value=300,
            tag="update_child_size",
            default_value=1,
        )

        dpg.add_button(
            label="Update child",
            callback=lambda: object_actions.update_child(
                object_actions.get_child_point()
            ),
        )
